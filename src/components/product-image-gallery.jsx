...
export default function ProductImageGallery({
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      <div
        ref={mainRef}
        className={`group relative overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-sm ${compact ? 'min-h-[300px]' : 'aspect-square'}`}
      ...
      >
        ...
        {hasMultiple && (
          <>
            <button
              type="button"
              onClick={() => move(-1)}
              className="absolute left-3 top-1/2 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full bg-white/92 text-slate-900 shadow-lg ring-1 ring-slate-200 transition hover:scale-105"
            ...
            <button
              type="button"
              onClick={() => move(1)}
              className="absolute right-3 top-1/2 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full bg-white/92 text-slate-900 shadow-lg ring-1 ring-slate-200 transition hover:scale-105"
        ...
        {normalizedImages.length > 0 && (
          <div className="absolute bottom-4 right-4 rounded-full bg-white/92 px-3 py-1.5 text-xs font-black text-slate-800 shadow-sm ring-1 ring-slate-200">
          ...
          {normalizedImages.map((image, index) => (
            <button
              ...
              className={`relative h-20 w-20 shrink-0 overflow-hidden rounded-2xl border-2 bg-white p-1 transition ${
                index === activeIndex
                  ? 'border-[var(--shop-primary,#4f46e5)] shadow-md ring-2 ring-[var(--shop-primary,#4f46e5)]/15'
