...
export function normalizeProductImages(images, fallbackImage = null) {
  ...
  if (typeof images === 'string') {
    try { source = JSON.parse(images) } catch { source = images.split(',') }
  }
...
export default function ProductImageGallery({
  ...
  useEffect(() => {
    setActiveIndex(0)
    setZoom({ active: false, x: 50, y: 50, left: 50, top: 50 })
    ...
    <div className={`space-y-3 ${className}`}>
      ...
      >
        ...
        {normalizedImages.length > 0 && (
          <div className="absolute bottom-4 right-4 rounded-lg bg-white/94 px-3 py-1.5 text-xs font-black text-slate-800 shadow-sm ring-1 ring-slate-200">
