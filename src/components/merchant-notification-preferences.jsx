...
export function MerchantNotificationPreferences() {
  ...
  async function load() {
    ...
    setLoading(true)
    const [{ data, error }, { data: deliveryLogs, error: logsError }] = await Promise.all([
    ...
    if (logsError) toast.warning('Preferences loaded, but recent delivery logs are unavailable.')
  ...
  async function save() {
    ...
    const { error } = await supabase.from('merchant_notification_preferences').upsert({
      ...
      weekly_report: false, marketing: prefs.marketing,
  ...
  const types = [
    ['new_order','New orders','Instant dashboard, email or SMS when a customer places an order.',Bell,true],
    ['low_stock','Low-stock alerts','Triggered when a product crosses its low-stock threshold or reaches zero.',AlertTriangle,true],
    ['order_status','Order status updates','Dashboard, email or SMS when an order status changes.',RefreshCw,true],
    ['pending_order_reminder','48-hour pending-order reminders','Alerts you once when an order has remained Pending for more than 48 hours.',AlertTriangle,true],
    ['weekly_report','Weekly report','Coming soon. This stays off until the scheduled weekly report worker is available.',Mail,false],
    ['marketing','BazarHQ tips and updates','Optional product and platform communication.',MessageSquare,true],
  ]
  ...
  return <div className="space-y-5">
    ...
    <section className="rounded-2xl border border-border bg-card p-6"><h3 className="font-semibold">Notification types</h3><div className="mt-4 divide-y divide-border">{types.map(([key,label,desc,Icon,available]) => <div key={key} className="flex items-center justify-between gap-4 py-4"><div className="flex gap-3"><Icon className="mt-0.5 h-4 w-4 text-muted-foreground" /><div><div className="flex items-center gap-2"><p className="text-sm font-medium">{label}</p>{!available && <span className="rounded-full bg-muted px-2 py-0.5 text-[10px] font-semibold text-muted-foreground">Coming soon</span>}</div><p className="mt-0.5 text-xs leading-5 text-muted-foreground">{desc}</p></div></div><Switch checked={available ? Boolean(prefs[key]) : false} disabled={!available} onCheckedChange={(value) => available && set(key,value)} /></div>)}</div><Button onClick={save} disabled={saving} className="mt-5 gap-2">{saving && <Loader2 className="h-4 w-4 animate-spin" />}Save preferences</Button></section>
