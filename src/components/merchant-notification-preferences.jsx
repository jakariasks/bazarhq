...
const DEFAULTS = {
  new_order: true, low_stock: true, order_status: true, weekly_report: false, marketing: false,
  dashboard_enabled: true, email_enabled: true, sms_enabled: false, sms_email_fallback: true,
...
export function MerchantNotificationPreferences() {
  ...
  async function save() {
    ...
    if (prefs.sms_enabled && !/^(?:880)?01[3-9][0-9]{8}$/.test(phone)) return toast.error('Enter a valid Bangladesh notification phone number.')
    ...
    const { error } = await supabase.from('merchant_notification_preferences').upsert({
      ...
      new_order: prefs.new_order, low_stock: prefs.low_stock, order_status: prefs.order_status,
  ...
  const types = [
    ...
    ['order_status','Order status updates','Merchant dashboard notice when an order status changes.',RefreshCw],
  ...
  return <div className="space-y-5">
    <section className="rounded-2xl border border-border bg-card p-6">
      <h3 className="font-semibold">Delivery channels</h3><p className="mt-1 text-sm text-muted-foreground">Dashboard uses Supabase Realtime. Email and SMS are processed by retryable delivery queues.</p>
      ...
      <label className="mt-4 flex items-center justify-between rounded-xl bg-muted/40 p-4"><span><span className="block text-sm font-medium">Email fallback after final SMS failure</span><span className="text-xs text-muted-foreground">The processor retries first, then queues an email when SMS reaches its maximum attempts.</span></span><Switch checked={prefs.sms_email_fallback} onCheckedChange={(value) => set('sms_email_fallback',value)} /></label>
    </section>
    ...
    <section className="rounded-2xl border border-border bg-card p-6"><div className="flex items-center justify-between"><div><h3 className="font-semibold">Recent delivery attempts</h3><p className="mt-1 text-xs text-muted-foreground">Safe provider status and failure logs; recipients are masked.</p></div><Button variant="outline" size="sm" onClick={load}>Refresh</Button></div><div className="mt-4 divide-y divide-border overflow-hidden rounded-xl border">{logs.length ? logs.map((log) => <div key={log.id} className="flex flex-col gap-1 p-3 text-sm sm:flex-row sm:items-center"><span className="font-medium uppercase">{log.queue_type}</span><span className="text-muted-foreground">{log.notification_type || 'notification'} · {log.recipient_masked || 'masked'}</span><span className={`sm:ml-auto ${log.status === 'sent' ? 'text-success' : log.status === 'failed' ? 'text-destructive' : 'text-amber-600'}`}>{log.status} · attempt {log.attempt}</span>{log.error_message && <span className="text-xs text-destructive sm:basis-full">{log.error_message}</span>}</div>) : <p className="p-5 text-center text-sm text-muted-foreground">No delivery attempts logged yet.</p>}</div></section>
  </div>
}
