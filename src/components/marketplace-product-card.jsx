...
export default function MarketplaceProductCard({
  ...
  return (
    <motion.article
      ...
      className={`group flex h-full flex-col overflow-hidden rounded-[1.15rem] border border-slate-200/90 bg-white shadow-[0_16px_42px_-30px_rgba(15,23,42,.32)] transition-[transform,border-color,box-shadow] duration-500 ease-out hover:-translate-y-1.5 hover:border-indigo-200 hover:shadow-[0_30px_72px_-34px_rgba(79,70,229,.34)] focus-within:-translate-y-1 focus-within:border-indigo-200 focus-within:shadow-[0_26px_64px_-34px_rgba(79,70,229,.28)] ${className}`}
    >
      <Link
      ...
      <div className="flex flex-1 flex-col p-3.5 sm:p-4">
        <div className="flex items-center justify-between gap-2 text-[10px] font-bold uppercase tracking-[0.11em] text-slate-400 sm:text-[11px]">
        ...
        <Link to="/shop/$storeSlug/product/$productId" params={{ storeSlug, productId }} className="mt-1.5 block">
          <h3 className="line-clamp-2 min-h-[2.45rem] text-[14px] font-black leading-[1.2rem] tracking-[-0.012em] text-slate-950 transition-colors hover:text-indigo-700 sm:text-[15px] sm:leading-[1.28rem]">
        ...
        <div className="mt-2 flex items-end justify-between gap-3">
          <div className="min-w-0">
            <p className="text-xl font-black leading-none tracking-[-0.025em] text-slate-950 sm:text-[1.3rem]">{money(price, currency)}</p>
          ...
          <span className={`shrink-0 rounded-full px-2.5 py-1 text-[9px] font-black sm:text-[10px] ${outOfStock ? 'bg-rose-50 text-rose-600' : sold > 0 ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>
            {outOfStock ? 'Out of stock' : sold > 0 ? `${sold.toLocaleString('en-BD')} sold` : 'New'}
          </span>
        </div>

        <div className="mt-auto pt-3">
          <div className="flex items-center justify-between gap-2 border-t border-slate-100 pt-3 text-[10px] font-semibold text-slate-500 sm:text-xs">
          ...
          {comparison && saving > 0 && (
            <p className="mt-2 text-[10px] font-bold text-emerald-700">Save up to {money(saving, currency)}</p>
          )}
          {statusMessage && <p className={`mb-2 rounded-lg px-2.5 py-2 text-[10px] font-bold ${statusTone === 'error' ? 'bg-rose-50 text-rose-600' : 'bg-emerald-50 text-emerald-700'}`}>{statusMessage}</p>}
          {onAddToCart && (
            <button
              type="button"
              disabled={addToCartDisabled}
              onClick={handleAdd}
              className="mt-3 h-10 w-full rounded-xl bg-slate-950 px-4 text-xs font-black text-white transition hover:bg-indigo-600 disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-500"
            >
              {addToCartLabel}
            </button>
          )}
