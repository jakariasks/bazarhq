...
function Onboarding() {
  ...
  // suggested cats change when shopType changes
  ...
  // When shop type changes, reset categories to all of that type by default
  const onShopTypeChange = (t) => {
    setShopType(t)
    setCategories([]) // reset — let merchant pick
  }
  ...
  const launch = async () => {
    ...
    if (error || !data) { setLaunching(false); toast.error(error?.message ?? 'Failed to create store'); return }
          ...
          <motion.div key={step} initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: 0.22 }}
            ...
            {step === 1 && (
              <>
                ...
                <div className="mt-6 grid gap-2">
                  ...
                  <Input autoFocus value={shopName}
                    onChange={(e) => { setShopName(e.target.value); if (!touched) setSub(slugify(e.target.value)) }}
                    placeholder="My Awesome Shop" maxLength={80} className="h-11 text-base" />
                </div>
              </>
            )}
              ...
              <>
                ...
                <div className="mt-6 grid gap-2">
                  ...
                  <div className={`flex overflow-hidden rounded-lg border bg-white transition-all focus-within:ring-2 ${
                    ...
                    <Input autoFocus value={sub} onChange={(e) => onSubInput(e.target.value)}
                      onBlur={() => setTouched(true)} autoComplete="off" spellCheck={false}
                      placeholder="my-shop" className="h-11 border-0 text-base focus-visible:ring-0" />
            ...
            {step === 3 && (
              <>
                ...
                {/* Categories — shown only after shop type selected */}
                <AnimatePresence>
                  {shopType && (
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} transition={{ duration: 0.2 }}
                      ...
                      <div className="mb-3 flex items-center justify-between">
                        ...
                        <div className="flex gap-2">
                          <button type="button" onClick={() => setCategories(suggestedCats)}
                      ...
                      <div className="mt-3 flex gap-2">
                        <Input value={catInput} onChange={(e) => setCatInput(e.target.value)}
                          ...
                          placeholder="Add a custom category…" className="flex-1" />
                        ...
                        <div className="mt-2 flex flex-wrap gap-2">
                          {categories.filter((c) => !suggestedCats.includes(c)).map((c) => (
                            <span key={c} className="flex items-center gap-1 rounded-full bg-accent/20 px-3 py-1 text-xs font-medium text-accent-foreground">
                              {c}
                              <button type="button" onClick={() => removeCategory(c)} className="hover:text-destructive"><X className="h-3 w-3" /></button>
                            </span>
                          ))}
                        </div>
                      )}
                      ...
                      {categories.length === 0 && (
                        <p className="mt-2 text-xs text-destructive">Please select at least one category.</p>
                      )}
                    </motion.div>
                  )}
                </AnimatePresence>
              ...
              <div className="flex flex-col items-center py-6 text-center">
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                  ...
                  <p className="mt-2 text-sm text-muted-foreground">
                    Your new store will live at <strong className="text-foreground">{sub}.bazarhq.com</strong>
                  </p>
                  <div className="mt-4 space-y-2">
                    <div className="flex flex-wrap items-center justify-center gap-2">
                      <span className="rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold text-primary">{shopType}</span>
                    </div>
            ...
            <div className="mt-8 flex items-center justify-between">
              ...
              {step < 5 ? (
                ...
                <Button onClick={launch} disabled={launching} className="bg-gradient-primary px-8 shadow-glow">
                  {launching ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Creating…</> : <>Launch my shop <ArrowRight className="ml-1.5 h-4 w-4" /></>}
                </Button>
              )}
            </div>
          </motion.div>
...
function WrappedOnboarding() { return <AuthGuard><Onboarding /></AuthGuard> }
