...
// Validate Bangladeshi phone: 11 digits, starts with 01
function validateBDPhone(raw) {
  const digits = raw.replace(/\D/g, '')
  if (digits.length !== 11) return { ok: false, message: 'Must be 11 digits (e.g. 01306060688)' }
  if (!/^01[3-9]/.test(digits)) return { ok: false, message: 'Must start with 013–019' }
  return { ok: true, digits }
}

// Convert 01XXXXXXXXX → +8801XXXXXXXXX for Supabase
            ...
            {tab === 'email' && (
              ...
              >
                <div className="grid gap-2">
                  ...
                  <Input id="name-e" required placeholder="Jakaria Hasan" value={name} onChange={(e) => setName(e.target.value)} />
                </div>
                <div className="grid gap-2">
                  <Label htmlFor="email">Email</Label>
                  <Input id="email" type="email" required placeholder="jakaria@gmail.com" value={email} onChange={(e) => setEmail(e.target.value)} />
                </div>
                <PasswordFields
            ...
            {tab === 'phone' && (
              ...
              >
                <div className="grid gap-2">
                  ...
                  <Input id="name-p" required placeholder="Jakaria Hasan" value={name} onChange={(e) => setName(e.target.value)} />
                </div>

                <div className="grid gap-2">
                  <Label htmlFor="phone">Phone number</Label>
                  <div className={`flex overflow-hidden rounded-lg border bg-background transition-all focus-within:ring-2 ${
                  ...
                  {phoneError ? (
                    ...
                    <p className="text-xs text-muted-foreground">
                      Bangladeshi mobile only. Example: 01306060688.
                    </p>
                  )}
                </div>
