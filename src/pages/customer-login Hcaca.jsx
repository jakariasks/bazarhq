...
export default function CustomerLoginPage() {
  ...
  async function handleLogin(event) {
    ...
    try {
      await signIn({ email: emailCheck.email, password, captchaToken })
  ...
  return (
    <AuthShell>
      ...
      <div className="rounded-[2rem] border border-border/80 bg-card/95 p-6 text-card-foreground shadow-xl shadow-slate-200/70 backdrop-blur sm:p-8">
        ...
        <Button type="button" variant="outline" className="h-11 w-full gap-2 rounded-xl" onClick={handleGoogleLogin} disabled={googleLoading || loading}>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <div>
            ...
            <div className="relative mt-1">
              <Input id="customerPassword" type={showPassword ? 'text' : 'password'} autoComplete="current-password" placeholder="Password" value={password} onChange={(event) => setPassword(event.target.value)} required className="h-11 rounded-xl pr-10" />
        ...
        <p className="mt-6 text-center text-sm text-muted-foreground">
          New customer?{' '}
          <Link to="/customer/signup" search={signupSearch} className="font-semibold text-primary hover:underline">Create account</Link>
        </p>
      </div>
    </AuthShell>
  )
