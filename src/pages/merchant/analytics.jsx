...
import { TrendingUp, ShoppingCart, Package, Users, Download, Calendar, BarChart3, Eye, Percent, Home, Tags, Clock3, ExternalLink, CheckCircle2, AlertTriangle, Trophy } from 'lucide-react'
...
function toBounds(startText, endText) {
  ...
  const endInclusive = new Date(`${endText}T23:59:59.999`)
  return { start, end: new Date(endInclusive.getTime() + 1) }
}
function validateRange(startText, endText) {
  if (!startText || !endText) return 'Choose both start and end dates.'
  const { start, end } = toBounds(startText, endText)
  if (Number.isNaN(start.getTime()) || Number.isNaN(end.getTime()) || start >= end) return 'Choose a valid date range.'
  if (end.getTime() - start.getTime() > MAX_RANGE_MS) return 'Analytics range cannot exceed five years.'
...
export default function AnalyticsPage() {
  ...
  const rangeError = useMemo(() => validateRange(startDate, endDate), [startDate, endDate])
  ...
  const { data: response, isLoading, error, refetch, dataUpdatedAt } = useQuery({
    queryKey: ['merchant-analytics-v3', store?.id, startDate, endDate],
    enabled: !!store?.id && !rangeError,
    queryFn: async () => {
      const started = performance.now()
      const controller = new AbortController()
      const timeout = window.setTimeout(() => controller.abort(), QUERY_TIMEOUT_MS)
      try {
        const query = supabase.rpc('get_merchant_analytics', {
          p_store_id: store.id,
          p_start: bounds.start.toISOString(),
          p_end: bounds.end.toISOString(),
        })
        const { data, error: rpcError } = typeof query.abortSignal === 'function'
  ...
  async function exportCSV() {
    ...
    if (!response?.payload) return toast.error('Load analytics before exporting.')
    ...
    try {
      ...
      const exported = {
        orders: headerNumber(exportResponse, 'x-row-count'),
        validOrders: headerNumber(exportResponse, 'x-valid-orders'),
        cancelledOrders: headerNumber(exportResponse, 'x-cancelled-orders'),
        revenue: headerNumber(exportResponse, 'x-analytics-revenue'),
        uniqueVisitors: headerNumber(exportResponse, 'x-unique-visitors'),
        productViews: headerNumber(exportResponse, 'x-product-views'),
      }
      const mismatch = []
      if (exported.orders != null && exported.orders !== Number(summary.orders || 0)) mismatch.push('order count')
      if (exported.validOrders != null && exported.validOrders !== Number(summary.valid_orders || 0)) mismatch.push('valid-order count')
      if (exported.cancelledOrders != null && exported.cancelledOrders !== Number(summary.cancelled_orders || 0)) mismatch.push('cancelled-order count')
      if (exported.revenue != null && !nearlyEqual(exported.revenue, summary.revenue)) mismatch.push('revenue')
      if (exported.uniqueVisitors != null && exported.uniqueVisitors !== Number(summary.unique_visitors || 0)) mismatch.push('unique visitors')
      if (exported.productViews != null && exported.productViews !== Number(summary.product_views || 0)) mismatch.push('product views')
      if (mismatch.length) throw new Error(`Export reconciliation failed for ${mismatch.join(', ')}. Refresh analytics and try again.`)
      ...
      toast.success(`CSV exported and reconciled with dashboard totals (${exported.orders ?? 0} orders).`)
    } catch (exportError) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div><h1 className="text-2xl font-semibold tracking-tight sm:text-3xl">Analytics</h1><p className="mt-1 text-sm text-muted-foreground">Server-aggregated revenue, orders, unique sessions, conversion, product sales and storefront traffic.</p></div>
        <div className="flex flex-wrap items-end gap-2">
          ...
          <Button variant="outline" onClick={() => refetch()} disabled={isLoading || !!rangeError}>Refresh</Button>
          <Button variant="outline" onClick={exportCSV} disabled={exporting || isLoading || !!rangeError || !response?.payload} className="gap-2">{exporting ? <Clock3 className="h-4 w-4 animate-spin" /> : <Download className="h-4 w-4" />}Export CSV</Button>
        </div>
      </div>

      {rangeError && <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><AlertTriangle className="mr-2 inline h-4 w-4" />{rangeError}</div>}
      {error && <div className="rounded-2xl border border-destructive/30 bg-destructive/5 p-4 text-sm text-destructive">{error.name === 'AbortError' ? 'Analytics request timed out. Try a shorter date range.' : error.message}</div>}
      ...
      {!isLoading && response && <div className={`flex flex-wrap items-center gap-2 rounded-xl border px-4 py-3 text-xs ${slaMet ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-amber-200 bg-amber-50 text-amber-900'}`}>
        {slaMet ? <CheckCircle2 className="h-4 w-4" /> : <AlertTriangle className="h-4 w-4" />}
        <strong>Analytics load:</strong> {response.loadMs} ms · target ≤ {ANALYTICS_SLA_MS} ms
        {!slaMet && <span>Performance target missed; use the verification script/SQL to inspect indexes and query plan.</span>}
      </div>}
      ...
      {!isLoading && !hasData ? <div className="rounded-2xl border border-dashed border-border bg-muted/20 px-6 py-20 text-center"><BarChart3 className="mx-auto h-12 w-12 text-muted-foreground" /><h3 className="mt-4 text-lg font-semibold">No analytics in this date range</h3><p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-muted-foreground">Publish the storefront, share its link, and add products. Page views, category visits and orders will appear here after customers interact with the shop.</p></div> : <>
        {!hasOrders && <section className="rounded-2xl border border-dashed border-violet-200 bg-violet-50/60 px-6 py-12 text-center"><ShoppingCart className="mx-auto h-11 w-11 text-violet-500" /><h3 className="mt-4 text-lg font-semibold">No orders in this date range yet</h3><p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-muted-foreground">Traffic analytics are available below, but order/revenue graphs stay hidden until the first order arrives. Share your storefront, review product availability, and confirm at least one payment method is enabled.</p><a href="/merchant/products" className="mt-4 inline-flex items-center gap-1 text-sm font-semibold text-violet-700 hover:underline">Review catalog <ExternalLink className="h-3.5 w-3.5" /></a></section>}
        {hasOrders && <div className="grid gap-5 xl:grid-cols-[1.6fr_1fr]">
          <section className="rounded-2xl border border-border bg-card p-5"><div className="mb-4"><h2 className="font-semibold">Revenue and orders</h2><p className="text-xs text-muted-foreground">Daily values within the selected range; cancelled-order revenue is excluded.</p></div><div className="h-80"><ResponsiveContainer width="100%" height="100%"><LineChart data={daily}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="date" tickFormatter={(v) => new Date(v).toLocaleDateString('en-BD', { month:'short', day:'numeric' })} minTickGap={30} /><YAxis /><Tooltip formatter={(value, name) => name === 'revenue' ? currency(value) : value} /><Line type="monotone" dataKey="revenue" stroke="#6366F1" strokeWidth={3} dot={false} /><Line type="monotone" dataKey="orders" stroke="#10B981" strokeWidth={2} dot={false} /></LineChart></ResponsiveContainer></div></section>
        ...
        <div className="grid gap-5 xl:grid-cols-3">
          <section className="rounded-2xl border border-border bg-card p-5"><h2 className="font-semibold">Top-selling products</h2><div className="mt-4 divide-y divide-border">{sellingProducts.length ? sellingProducts.slice(0,10).map((product, index) => <div key={product.product_id || `${product.title}-${index}`} className="flex items-center gap-3 py-3"><span className="flex h-7 w-7 items-center justify-center rounded-full bg-amber-100 text-xs font-bold text-amber-800">{index + 1}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{product.title || 'Product'}</p><p className="text-xs text-muted-foreground">{Number(product.orders || 0)} order{Number(product.orders || 0) === 1 ? '' : 's'} · {currency(product.sales_value)}</p></div><span className="inline-flex items-center gap-1 text-sm font-semibold"><Trophy className="h-3.5 w-3.5 text-amber-500" />{Number(product.quantity || 0)} sold</span></div>) : <p className="py-8 text-center text-sm text-muted-foreground">No product sales in this range.</p>}</div></section>
